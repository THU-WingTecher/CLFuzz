...
std::string ToSeed(const Buffer& buffer){
    std::stringstream ss;
...
std::string ToSeed(const component::Ciphertext& ciphertext){
    std::stringstream ss;
    const size_t len = ciphertext.ciphertext.GetSize();
    unsigned char *data = (unsigned char*)ciphertext.ciphertext.GetPtr();
    for(size_t i=0; i<len;i++){
        ss <<  std::dec << (int)(data[i]);
        ss << " ";
    }
...
std::string ToSeed(const bool val){
    return util::ToString(val);
}
std::string ToSeed(const component::ECC_PublicKey& val){
    std::stringstream ss;
    ss << util::ToString(val.first);
    ss << " ";
    ss << util::ToString(val.second); 
    return util::ToString(val);
}
std::string ToSeed(const component::ECC_KeyPair& val){
    return util::ToString(val);
}
std::string ToSeed(const component::ECDSA_Signature& val){
    return util::ToString(val);
}
std::string ToSeed(const component::Bignum& val){
    return util::ToString(val);
}
std::string ToSeed(const component::G2& val){
    return util::ToString(val);
}
std::string ToSeed(const component::BLS_Signature& val){
    return util::ToString(val);
}
std::string ToSeed(const component::BLS_KeyPair& val){
    return util::ToString(val);
}
